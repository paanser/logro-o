...
function login() {
  ...
  if (input.value.trim() === PASSWORD) {
    ...
    // Retraso necesario en Safari para cargar correctamente los CSV
    setTimeout(loadCSVs, 200);
  } else {
...
async function loadCSVs() {
  try {
    ...
    alert('⚠️ No se pudieron cargar los CSV. Verifica que estén en la misma carpeta.');
  }
}
...
function calcularArea() {
  ...
  area_corregida = tablaMultiplos[ancho_corr]?.[alto_corr] || (ancho_corr * alto_corr) / 10000;
...
function calcularPrecio() {
  ...
  document.getElementById('resultadoPrecio').innerHTML =
    `<b>Precio m²:</b> ${precio_m2.toFixed(2)} €<br><b>Precio total:</b> ${precio_total.toFixed(2)} €`;
}
