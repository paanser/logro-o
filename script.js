/* VIDRES SOSA — SCRIPT PRINCIPAL v7 (manual funcional y validado) */
...
function initApp(){
  ...
  document.getElementById("btnCalcularManual").addEventListener("click", ()=>{
    ...
    if (!pBase){ alert("Introduce el precio €/m²"); return; }
    ...
    const resumen = `
      <div class="bloque-vidrio">
        ...
        Precio vidrio: ${pVidFinal.toFixed(2)} €/m²<br>
        Cantos: ${ml.toFixed(2)} m × ${pCanto.toFixed(2)} € = ${costeCantos.toFixed(2)} €<br>
        Unidades: ${uds}<br>
        Subtotal: ${subtotal.toFixed(2)} € — IVA: ${iva.toFixed(2)} €<br>
        <b>Total: ${total.toFixed(2)} €</b>
      </div><hr>`;

    outManual.innerHTML += resumen;

    listaPresupuesto.push({ nombreVid1:`Manual (${pBase.toFixed(2)} €/m²)`, nombreVid2:"", area, uds, subtotal, iva, total });
    mostrarPresupuesto();
  ...
  function mostrarPresupuesto(){
    ...
    outTarifa.innerHTML = html + `<h3>Total general: ${totalFinal.toFixed(2)} €</h3>`;
  }
  ...
  document.getElementById("btnExportarPDF").addEventListener("click", ()=>{
    ...
    texto += `TOTAL GENERAL: ${totalFinal.toFixed(2)} €`;
